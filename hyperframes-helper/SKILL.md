...
## Voiceover-Driven Pacing (non-negotiable)

**Whenever a video uses voiceover narration, the voice sets the ritmo. Everything else — video cuts, scene holds, animations, motion graphics, music — follows the voice. Never the other way around.** This applies at every Level (1, 2, 3) and to every element in the composition.

- **Generate/measure the real VO audio first.** Build the timeline (`data-start`/`data-duration` on every clip) from the VO's actual measured duration (`ffprobe`, see `marcus-audio`'s Hard Rule #4), not from a draft estimate or a planned scene length decided in advance.
- **Never speed up, slow down, re-time, re-cut, or truncate the voiceover to fit a scene, an animation, or a music beat.** The voiceover clip is the one thing in the composition that is never adjusted for sync. If something doesn't fit against it, that something is what changes — not the voice.
  - Motion graphics running long against the VO? Shorten or simplify the animation.
  - VO running long against a planned visual beat? Extend that scene's `data-duration` to match the VO's real length — don't trim the audio to fit the shorter hold you'd originally planned.
- **Music and animation timing are the flexible layers.** They can compress, loop, extend, fade, or restart to land on VO boundaries. The VO itself never compresses or stretches to land on them — see `marcus-audio`'s mixing pattern (music bed as one long `<audio>` clip trimmed/faded to the VO-driven total length, narration clips untouched).
- **Word-count pacing (~2.2–2.5 words/sec spoken Spanish) is a drafting estimate only**, useful before any VO file exists to rough out a storyboard or shot list. The moment a real VO clip exists, throw the estimate away and rebuild the timeline from its measured duration.
- Applies at every Level: in Level 2 storyboards, a VO-driven scene's motion notes should say "paced to VO line N" rather than a fixed duration guess; in Level 3 Step 02, title cards are timed against the transcript's real word-level timestamps (from `templates/transcribe-whisper.py`), not an assumed reading speed.

See `marcus-audio`'s "WORKFLOW: narration + music for a Hyperframes tutorial" section for the concrete generate → measure → build-timeline → mix pipeline this rule governs.
