...
Dark, dense **Operate-mode** dashboard aesthetic (per `impeccable`'s Operate depth: scanability and consistency outrank expression) with a Linear-inspired visual language: near-black neutrals (`#0a0a0c`), a single restrained indigo accent (`#6672e5`) reserved for primary actions/selection/links, and semantic state color kept strictly separate from it — teal=done, amber=in-progress, rose=blocked, slate=dependency. One typeface (Inter) carries headings, labels, and data; monospace is used only for genuinely tabular/measurement values (dates, commit hash, IDs), not as a "technical" costume.
...
- **No decorative progress ring, no KPI card grid.** The header shows a plain number + one segmented progress bar; rollup stats are a single row of inline chips, not six bordered boxes.
...
- Flat elevation only: border-only cards, no shadows; hover changes border-color/background, never `translateY`. Focus-visible outlines, themed `::selection`, and a themed scrollbar are already wired — keep them if you edit the CSS.

It commits to a single dark theme on purpose (background + all colors painted explicitly, so it holds on any host) — this is the look the panel was validated against (rendered via headless Chrome during development); keep it unless the user asks for a reskin. If they do, the CSS `:root` tokens at the top are the only thing to swap.
